            ...
            <div data-anim-wrap class="row y-gap-30 pt-50">
                @if ($categories)
                    @foreach($categories as $categ)                    
                        ...
                        <?php
                        $categoriesz = TrainingCategories::where('status',1)->where('parent_id',$categ->id)->get();        
                        if(count($categoriesz) > 0){
                           ?>
                            <a href="{{ route('training-categories',['slug' => $categ->slug ]) }}" class="categoryCard -type-4"> 
                            <?php
                        }else{
                           ?>
